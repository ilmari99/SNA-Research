...
#SBATCH --job-name=blokus_simulate
#SBATCH --account=project_2010270
#SBATCH --time=00:12:00
#SBATCH --partition=test
#SBATCH --output=blokus_simulate_%j.out
#SBATCH --error=blokus_simulate_%j.err
...
for node in $(scontrol show hostname $SLURM_JOB_NODELIST); do
    ...
    srun --nodes=1 --ntasks=1 --cpus-per-task=256 --hint=multithread -w $node $PYTHON_EXE ./Blockus/only_simulate.py \
