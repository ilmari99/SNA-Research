...
def game_constructor(i):
    model_paths = list(filter(lambda path: path.endswith(".tflite"), os.listdir("/home/ilmari/python/RLFramework/BlockusModels")))
    model_paths = [os.path.abspath(f"/home/ilmari/python/RLFramework/BlockusModels/{model_path}") for model_path in model_paths]
    return BlockusGame(
        board_size=(14,14),
        timeout=40,
        logger_args = None,
...
def players_constructor(i, model_path):
    ...
    all_model_paths = [os.path.abspath(f"/home/ilmari/python/RLFramework/BlockusModels/model_{i}.tflite") for i in range(epoch_num + 1)]
...
def model_fit(ds, epoch, num_samples):
    ...
    train_ds = ds.take(int(0.7*num_samples)).batch(32)
    val_ds = ds.skip(int(0.7*num_samples)).batch(32)
    ...
    if epoch == 0:
        ...
        board = tf.keras.layers.Conv2D(16, (3,3), activation='relu')(board)
        board = tf.keras.layers.Conv2D(32, (2,2), activation='relu')(board)
        board = tf.keras.layers.Conv2D(64, (2,2), activation='relu')(board)
        ...
        model = tf.keras.models.load_model(f"/home/ilmari/python/RLFramework/BlockusModels/model_{epoch-1}.keras")
    ...
    return os.path.abspath(f"/home/ilmari/python/RLFramework/BlockusModels/model_{epoch}.keras")

if __name__ == "__main__":
    fit_model(players_constructor,
              game_constructor,
              model_fit,
              starting_model_path="/home/ilmari/python/RLFramework/BlockusModels/model_0.tflite",
              num_epochs=10,
              num_games=20,
              num_files=-1,
              num_cpus=10,
