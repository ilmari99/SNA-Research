...
def benchmark_all_tflite_files_in_folder(folder, num_games, num_proc, num_files):
    ...
    for f in os.listdir(folder):
        if f.endswith(".tflite"):
            ...
            res = simulate_games(game_constructor,
                                 PickleableFunction(players_constructor, model_path=model_path),
                                 folder, num_games,
                                 num_files=num_files,
                                 num_cpus=num_proc,
                                 return_results=True
    ...
    # Sort and print
    sorted_model_performance = sorted(model_performance.items(), key=lambda x: x[1], reverse=True)
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--tflite_folder", help="Folder to save the data to", default="/home/ilmari/python/RLFramework/models")
    ...
    argparser.add_argument("--folder", help="Folder to save the data to", default="BenchmarkData")
    ...
    model_performance = benchmark_all_tflite_files_in_folder(argparser.tflite_folder, num_games, num_proc, num_files)
